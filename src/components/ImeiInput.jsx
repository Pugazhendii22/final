...
const ImeiInput = ({ value, onChange, label, placeholder = '', required = false, scannerId }) => {
  ...
  }, [scannerOpen, scannerId, onChange]);
    ...
    <div>
      ...
      <div className="mt-1 flex items-center gap-2">
        ...
        <button
          ...
          className="bg-slate-100 border p-2 rounded text-slate-700 hover:bg-slate-200"
        >
      ...
      {scannerOpen && (
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center" onClick={stopScanner}>
          <div className="bg-white rounded-xl p-4 max-w-sm w-full mx-4" onClick={(e) => e.stopPropagation()}>
            ...
            <div id={scannerId} className="w-full min-h-48 rounded-xl bg-slate-100" />
            <p className="mt-3 text-sm text-gray-600">Point camera at IMEI barcode</p>
            <div className="mt-4 text-right">
              <button type="button" onClick={stopScanner} className="px-4 py-2 bg-slate-100 rounded-md text-slate-800 hover:bg-slate-200">Cancel</button>
            </div>
          </div>
        </div>
      )}
